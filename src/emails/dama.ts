import data, { User } from "../data/dama";
import {
  readFileSync,
  existsSync,
  rmdirSync,
...
function getTemplate() {
  const template = readFileSync(
    resolve(__dirname, "../templates/dama-signature.mjml"),
    "utf8"
  );
...
function insertData(template: string, data: User) {
  try {
...
function createEmail(data: User) {
...
export default async function generate() {
  const path = resolve(__dirname, "../../emails/dama");
  if (existsSync(path)) {
